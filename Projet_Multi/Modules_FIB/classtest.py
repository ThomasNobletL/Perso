# -*- coding: utf-8 -*-
"""
Created on Thu Feb 15 09:41:23 2024
...
class NiAcquisition:
    ...
    def initial_voltage_setting(self,min_tension, max_tension, channel_ud):
        """
    ...
    def configure_tasks(self,channel_lr, channel_ud, channel_read, min_tension, max_tension, 
    ...
    def write_and_read(self,write_task, read_task, data_to_write,total_samples_to_read, timeout):
        """
    ...
    def stop(self,write_task, read_task):
        """
        Stops the write and read tasks.
    
        This function stops the specified write and read tasks, halting the writing of scanning signals
        and ending the reading of the input signal.
    
        Args:
            write_task (nidaqmx.Task): The write task to stop.
            read_task (nidaqmx.Task): The read task to stop.
    ...
    def run(self,min_tension, max_tension, channel_ud,channel_lr, channel_read, 
                        sampling_frequency, complete_horizontal_staircase, total_samples_to_read,
                        vertical_staircase,data_to_write,sample_to_read,timeout) :
        
        self.initial_voltage_setting(min_tension, max_tension, channel_ud)     
        write_task,read_task = self.configure_tasks(channel_lr, channel_ud, channel_read, min_tension, max_tension, 
                            sampling_frequency, complete_horizontal_staircase, total_samples_to_read,
                            vertical_staircase)
        self.write_and_read(write_task, read_task, data_to_write,total_samples_to_read, timeout)
        self.stop(write_task, read_task)